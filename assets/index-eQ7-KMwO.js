...
`)){const c=s.trim();if(!c){i=null;continue}if(c.startsWith("- ")&&i){const b=o[i],y=vn(c.slice(2).trim()),w=Array.isArray(b)?b:[];w.push(y),o[i]=w;continue}const l=s.indexOf(":");if(l===-1){i=null;continue}const d=s.slice(0,l).trim(),g=s.slice(l+1).trim();if(!g){o[d]=[],i=d;continue}o[d]=vn(g),i=null}return{frontmatter:o,body:r}}function vn(e){return e.startsWith("[")&&e.endsWith("]")?e.slice(1,-1).split(",").map(n=>n.trim().replace(/^['"]|['"]$/g,"")).filter(Boolean):e==="true"?!0:e==="false"?!1:e.replace(/^['"]|['"]$/g,"")}function te(e){return e.replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;").replace(/'/g,"&#39;")}function Pa(e){return/^(https?:|data:|blob:|\/)/.test(e)?e:`${"/".endsWith("/")?"/":"//"}${e.replace(/^\/+/,"")}`}function it(e){return/^https?:\/\//.test(e)}function Je(e){return e.trim().replace(/^@/,"").toLowerCase()}function Ua(e){var t;if(typeof e.url=="string")return e.url;if(typeof e.profileUrl=="string")return e.profileUrl;const n=(t=e.socials)==null?void 0:t.find(a=>a.social==="github");if(typeof(n==null?void 0:n.url)=="string")return n.url;if(typeof(n==null?void 0:n.username)=="string")return`https://github.com/${n.username.replace(/^@/,"")}`;if(typeof e.username=="string")return`https://github.com/${e.username.replace(/^@/,"")}`}function Fa(e,n){if(typeof e!="string"||!e.trim())throw new Error(`Invalid article author in ${n}`);const t=Je(e),a=Oa.find(r=>Je(r.username??r.name)===t||Je(r.name)===t);return a?{name:a.name,url:Ua(a),avatar:typeof a.avatar=="string"?a.avatar:void 0}:it(e)?{name:e,url:e}:{name:e}}function Ha(e,n){if(!/^(https?:\/\/|\/|\.\/|\.\.\/|[A-Za-z0-9/_-])/.test(n))return Ae(e);const t=te(n),a=it(n)?' target="_blank" rel="noreferrer"':"";return`<a href="${t}"${a}>${Ae(e)}</a>`}function Ae(e){return te(e).replace(/`([^`]+)`/g,"<code>$1</code>").replace(/\*\*([^*]+)\*\*/g,"<strong>$1</strong>").replace(/\*([^*]+)\*/g,"<em>$1</em>")}function Ga(e,n){let t=0;for(let a=n;a<e.length;a+=1){const r=e[a];if(r==="("){t+=1;continue}if(r===")"){if(t===0)return a;t-=1}}return-1}function re(e){let n="",t=0;for(;t<e.length;){const a=e.indexOf("[",t);if(a===-1){n+=Ae(e.slice(t));break}const r=e.indexOf("]",a+1);if(r===-1||e[r+1]!=="("){n+=Ae(e.slice(t,a+1)),t=a+1;continue}const o=r+2,i=Ga(e,o);if(i===-1){n+=Ae(e.slice(t,a+1)),t=a+1;continue}n+=Ae(e.slice(t,a)),n+=Ha(e.slice(a+1,r),e.slice(o,i)),t=i+1}return n}function za(e){const n=e.toLowerCase();return{js:"JavaScript",javascript:"JavaScript",json:"JSON",md:"Markdown",markdown:"Markdown",sh:"Shell",shell:"Shell",swift:"Swift",ts:"TypeScript",typescript:"TypeScript",yaml:"YAML",yml:"YAML"}[n]??(e?e[0].toUpperCase()+e.slice(1):"Code")}function Wa(e){var r;const n=e.match(/(?:^|\s)(?:title|filename)=["']([^"']+)["']/),t=((r=e.split(/\s+/)[0])==null?void 0:r.replace(/[^\w#+-]/g,""))??"",a=(n==null?void 0:n[1])??e.replace(t,"").trim().replace(/^["']|["']$/g,"");return{language:t,title:a}}function qa(e,n,t){const a=za(n);return`
...
struct PlayerSystem: System {
  ...
  `}async function Cs(e){const n=await ct(),t=ns(n,e);if(!t){Ge("Demo not found","Check the address or return to the demos page.");return}ln(ls(t));const a=await es(t),r=n.commit??"main",o=`https://github.com/${n.repository}/blob/${r}/${t.sourcePath}`;oe.innerHTML=`
    ${ce()}
      ...
      <article class="container content-restriction demo-detail-page">
        ...
        <section class="demo-source-section" aria-labelledby="demo-source-title">
          ...
          <figure class="article-code-block demo-source-code">
            ...
            <pre class="code-with-line-numbers"><code class="${at("swift")}">${$a(a,"swift")}</code></pre>
          </figure>
        </section>
      </article>
    </main>
  ...
  `}function Ws(e){return/^(https?:|data:|blob:|\/)/.test(e)||e.startsWith("images/")?V(e):V(`images/${e}`)}function In(e,n){return e.map(t=>`
      ...
      `).join("")}function qs(e){var r;if(!e.length)return"";const n=In(e,"desktop"),t=In(e,"mobile"),a=((r=e[0])==null?void 0:r.title)??"Start";return`
